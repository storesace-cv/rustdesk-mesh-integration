...
- `scripts/` – Scripts auxiliares (fluxo Step-* obrigatório):
  ...
  - `Step-4-collect-error-logs.sh` – junta logs de `logs/local/` e `logs/deploy/` após qualquer falha (incluindo no Step-5),
...
- `logs/` (local apenas):
  ...
  - Ignorado pelo Git para impedir sincronização acidental. Estrutura principal:
    ...
    - `logs/archive/run-<id>-logs-<timestamp>.tar.gz` + symlink `logs-latest.tar.gz` para o último bundle do Step-4 (inclui
